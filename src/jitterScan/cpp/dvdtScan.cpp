...
void dvdtScan( const char *in_file, double Dut_tmin, double Dut_tmax, double Dut_pmin, double Dut_pmax, double Trig_tmin, double Trig_tmax, double Trig_pmin, double Trig_pmax, std::string SensorName, std::string Bias, std::string Fluence, std::string Temperature, std::string fitFunction )
{
	gROOT->SetBatch(true);
	...
	for(int scan=0; scan<100; scan++)
	{
		...
		loadTree->Project( "preHistogram", Form("dvdt2[%d]", scan), cuts);
		...
		for ( int i = 0; i < 20; i++)
		{
			...
			if (i < 5)
			{
				...
				loadTree->Project( name, Form("dvdt2[%d]", scan), cuts);
		...
		loadTree->Project( "postHistogram", Form("dvdt2[%d]", scan), cuts);
		...
		if (true) //saving image and txt
      	{
      		...
      		std::ofstream ofile;
      		ofile.open ( Form("%s_dvdt_scan.txt", substr.c_str() ), std::ios::app);
      		...
      		if(header)
      		{
      			...
      			ofile << "CFD" << " " << "dvdt[mV/ps]" << " " << "Error[]" << std::endl;
      			header = false;
      		}
      		...
      		img->WriteImage(Form("%s_dvdt[%d].png", substr.c_str(), scan));
      		delete img;
      	}
	...
	if (true)
	{
		gROOT->ProcessLine(".! mkdir dvdt_scan_Plots");
		gROOT->ProcessLine(".! mv *.png dvdt_scan_Plots");
		gROOT->ProcessLine(".! mv *dvdt_scan.txt dvdt_scan_Plots");
	}
