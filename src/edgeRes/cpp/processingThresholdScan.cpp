...
int processingThresholdScan()
{
	...
	ReadList.open("Threhold_Runlist.txt");
	...
	while(ReadList >> placeholder >> Bias >> iFile >> dut_TminC >> dut_TmaxC >> dut_PminC >> dut_PmaxC >> trig_TminC >> trig_TmaxC >> trig_PminC >> trig_PmaxC)
	{
		...
		thresholdResScan(ifile_location, dut_TminC, dut_TmaxC, dut_PminC, dut_PmaxC, trig_TminC, trig_TmaxC, trig_PminC, trig_PmaxC, SensorName, Bias, Fluence, Temperature);
